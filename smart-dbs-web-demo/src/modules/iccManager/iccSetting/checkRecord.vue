<template>
  <topDownLayout>
    <div class="check-top-bar" slot="top">
    ...
    <div slot="down">
        ...
        <a-popover placement="bottom" trigger="click">
          <template slot="content">
            ...
            >
              ...
              <div>
                <a-checkbox value="deviceVerifyMode">
                  核验方式
                </a-checkbox>
              </div>
            </a-checkbox-group>
      ...
      >
        ...
        <div slot="deviceVerifyMode" slot-scope="record">
          {{
            record.deviceVerifyMode | chooseVertifyMode(deviceVerifyModeList)
          }}
        </div>
      </a-table>
      <div class="pagination-box-cla">
        <a-pagination
          ...
          "
          @change="pageChange(arguments[0], arguments[1], getCheckedRecordList)"
          @showSizeChange="
            pageChange(arguments[0], arguments[1], getCheckedRecordList)
          "
        ></a-pagination>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      checkedValues: ['temperature', 'maskStatus', 'deviceVerifyMode']
    }
  },
  methods: {
    ...
    getCheckedRecordList() {
      this.request('getIccVerifyRecordList', this.getParams(), this.pager)
        .then(data => {
          if (data.code === '00') {
    ...
    exportFn() {
      ...
      httpClient.downloadUrl('/iccVerifyRecord/exportExcel', {
        lang: 'zh-cn',
        payload: {
          ...params
        }
  ...
  computed: {
    showColumns() {
      ...
      this.checkedValues.forEach(item => {
        ...
        if (item === 'deviceVerifyMode') {
          result.push({
            title: '验证方式',
            key: 'deviceVerifyMode',
            scopedSlots: { customRender: 'deviceVerifyMode' }
          })
        }
      })
      return result
